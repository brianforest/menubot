...
Output schema (return ONLY this JSON, no markdown, no commentary):
{
  "restaurant": { "en": string, "zh": string },   // en: full official name; if a restaurant context is given in the prompt, use its full official name (do NOT shorten to a logo/sign title printed on the menu, e.g. use "Planter's at The Danna Langkawi" not just "Planter's"); else best guess; "" if unknown.
                                                   // zh: how Taiwan media would present the venue — translate the place/hotel part into its common 中文 rendering; TRANSLITERATE the venue's brand name into 中文 too (use an established Chinese name if one exists, else a natural transliteration), followed by the original name in HALF-WIDTH parentheses with half-width spaces around them — exactly the pattern '空格(原文)空格', e.g. "普蘭特 (Planter's) 餐廳"; NEVER full-width （）, whose padding looks too airy. Add the venue type (餐廳/酒吧/咖啡廳); reorder to Chinese convention (place+hotel first, venue last); half-width space before the transliterated brand too: "Planter's at The Danna Langkawi" → "蘭卡威達那酒店 普蘭特 (Planter's) 餐廳". NEVER copy the English unchanged and NEVER produce a literal「A 的 B」gloss like "The Danna Langkawi 的 Planter's".
